...
private:
    ...
    /**
     * @brief Inserts a FamilyMember as a child of the node with id == member.id_boss.
     *        Left child = first successor, right child = second successor.
    ...
    /**
     * @brief Finds the first available successor (alive, not in jail) using pre-order traversal.
     * @param node  Root of the sub-tree to search.
     * @return Pointer to the qualifying BTNode, or nullptr if none found.
     */
    BTNode<FamilyMember>* findFirstAvailable(BTNode<FamilyMember>* node) const;
     ...
     */
    BTNode<FamilyMember>* findNearestBossWithTwoSuccessors(BTNode<FamilyMember>* bossNode) const;
...
public:
    ...
    /**
     * @brief Updates all editable fields of a member (everything except id and id_boss).
    ...
    /**
     * @brief Serializes the tree back into a LinkedList for saving via CSVReader::save().
